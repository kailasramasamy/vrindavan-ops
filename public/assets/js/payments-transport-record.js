(function () {
  ...
  const recordId = window.TRANSPORT_PAYMENT_RECORD_ID;
  const selectedMonth = window.TRANSPORT_PAYMENT_SELECTED_MONTH || "";

  if (!recordId) {
    console.error("Transport payment record ID not found");
    return;
  }
  ...
  function updateSummary(record) {
    const monthlyCostEl = document.querySelector('[data-field="monthly-cost"]');
    const adjustmentsEl = document.querySelector('[data-field="adjustments"]');
    const netPayEl = document.querySelector('[data-field="net"]');

    if (monthlyCostEl) {
      monthlyCostEl.textContent = `₹${formatNumber(record.monthly_cost || 0)}`;
    }
    if (adjustmentsEl) {
