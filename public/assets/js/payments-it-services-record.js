(function () {
  ...
  const recordId = window.IT_SERVICES_PAYMENT_RECORD_ID;
  const selectedMonth = window.IT_SERVICES_PAYMENT_SELECTED_MONTH || "";

  if (!recordId) {
    console.error("IT services payment record ID not found");
    return;
  }
  ...
  async function handleUpdateStatus() {
    ...
    if (!status) {
      if (window.toastManager) {
        window.toastManager.error("Please select a status");
      } else if (window.showErrorToast) {
        window.showErrorToast("Please select a status");
      } else {
    ...
    try {
      ...
      if (data && data.success) {
        ...
        // Show success toast using the toast system
        if (window.toastManager) {
          window.toastManager.success("Payment status updated successfully");
        } else if (window.showSuccessToast) {
          window.showSuccessToast("Payment status updated successfully");
        } else {
      ...
      if (window.toastManager) {
        window.toastManager.error("Failed to update payment status");
      } else if (window.showErrorToast) {
        window.showErrorToast("Failed to update payment status");
      } else {
  ...
  async function handleUpdateRemarks() {
    ...
    try {
      ...
      if (data && data.success) {
        if (window.toastManager) {
          window.toastManager.success("Remarks saved successfully");
        } else if (window.showSuccessToast) {
          window.showSuccessToast("Remarks saved successfully");
        } else {
          console.log("Remarks saved successfully");
        }
      } else {
        const errorMsg = data?.error || "Failed to save remarks";
        if (window.toastManager) {
          window.toastManager.error(errorMsg);
        } else if (window.showErrorToast) {
          window.showErrorToast(errorMsg);
      ...
      if (window.toastManager) {
        window.toastManager.error("Failed to save remarks");
      } else if (window.showErrorToast) {
        window.showErrorToast("Failed to save remarks");
      } else {
  ...
  function showAddEntryModal() {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (data && data.success) {
          if (window.toastManager) {
            window.toastManager.success("Entry added successfully");
          } else if (window.showSuccessToast) {
            window.showSuccessToast("Entry added successfully");
          } else {
          ...
          const errorMsg = data?.error || "Failed to add entry";
          if (window.toastManager) {
            window.toastManager.error(errorMsg);
          } else if (window.showErrorToast) {
            window.showErrorToast(errorMsg);
        ...
        if (window.toastManager) {
          window.toastManager.error("Failed to add entry");
        } else if (window.showErrorToast) {
          window.showErrorToast("Failed to add entry");
        } else {
  ...
  async function handleDeleteEntry(entryId) {
    ...
    try {
      ...
      if (data && data.success) {
        if (window.toastManager) {
          window.toastManager.success("Entry deleted successfully");
        } else if (window.showSuccessToast) {
          window.showSuccessToast("Entry deleted successfully");
        } else {
          console.log("Entry deleted successfully");
        }
        location.reload();
      } else {
        const errorMsg = data?.error || "Failed to delete entry";
        if (window.toastManager) {
          window.toastManager.error(errorMsg);
        } else if (window.showErrorToast) {
          window.showErrorToast(errorMsg);
      ...
      if (window.toastManager) {
        window.toastManager.error("Failed to delete entry");
      } else if (window.showErrorToast) {
        window.showErrorToast("Failed to delete entry");
      } else {
  ...
  // Toast functions are now handled inline in each function
  // The toast system is loaded via the _toast partial
})();
