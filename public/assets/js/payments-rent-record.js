(function () {
  ...
  const recordId = window.RENT_PAYMENT_RECORD_ID;
  const selectedMonth = window.RENT_PAYMENT_SELECTED_MONTH || "";

  if (!recordId) {
    console.error("Rent payment record ID not found");
    return;
  }
  ...
  function updateSummary(record) {
    const monthlyRentEl = document.querySelector('[data-field="monthly-rent"]');
    const adjustmentsEl = document.querySelector('[data-field="adjustments"]');
    const netPayEl = document.querySelector('[data-field="net"]');

    if (monthlyRentEl) {
      monthlyRentEl.textContent = `₹${formatNumber(record.monthly_rent || 0)}`;
    }
    if (adjustmentsEl) {
      ...
      adjustmentsEl.className = `text-xl font-bold mt-2 ${adjustments >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`;
    }
    if (netPayEl) {
