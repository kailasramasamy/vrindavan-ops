(function () {
  const API_BASE = "/payments/api/rent";
  const DEFAULT_MONTH = new Date().toISOString().slice(0, 7);
  const STORAGE_KEY = "rentPaymentsState";
  ...
  const elements = {
    monthInput: document.getElementById("rentPaymentsMonth"),
    statusFilter: document.getElementById("rentPaymentsStatusFilter"),
    tableBody: document.getElementById("rentPaymentsTableBody"),
    loadingRow: document.getElementById("rentPaymentsLoadingRow"),
    emptyRow: document.getElementById("rentPaymentsEmptyRow"),
    headerRange: document.getElementById("rentPaymentsHeaderRange"),
    recalculateBtn: document.getElementById("recalculateCycleBtn"),
    summary: {
      dates: document.getElementById("cycleDates"),
      propertyCount: document.getElementById("cyclePropertyCount"),
      rentAmount: document.getElementById("cycleRentAmount"),
  ...
  function loadStateFromStorage() {
    try {
      ...
      console.error("Failed to load rent payments state", error);
    }
  }
  ...
  function saveStateToStorage() {
    try {
      ...
      console.error("Failed to save rent payments state", error);
    }
  }
  ...
  function init() {
    ...
    if (typeof window.RENT_PAYMENTS_DEFAULT_MONTH !== 'undefined' && window.RENT_PAYMENTS_DEFAULT_MONTH) {
      state.month = window.RENT_PAYMENTS_DEFAULT_MONTH;
    }
  ...
  async function loadCycleData() {
    ...
    if (elements.tableBody) {
      const existingRows = Array.from(elements.tableBody.querySelectorAll("tr:not(#rentPaymentsLoadingRow):not(#rentPaymentsEmptyRow)"));
  ...
  async function recalculateCycle() {
    ...
    try {
      ...
      if (data && data.success) {
        ...
        const errorMsg = data?.error || "Failed to recalculate rent payments";
      ...
      showErrorToast?.("Failed to recalculate rent payments: " + (error.message || "Unknown error"));
      if (elements.emptyRow) {
        elements.emptyRow.classList.remove("hidden");
      }
    } finally {
  ...
  function renderRecordsTable() {
    ...
    state.filteredRecords.forEach((record) => {
      ...
      const monthlyRent = Number(record.monthly_rent) || 0;
      ...
      row.innerHTML = `
        <td class="px-6 py-4 whitespace-nowrap">
          <div class="text-sm font-medium text-gray-900 dark:text-white">${escapeHtml(record.property_name || "—")}</div>
        </td>
        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white capitalize">
          ${escapeHtml(record.property_type || "—")}
        </td>
        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
          ${escapeHtml(record.property_location || "—")}
        </td>
        ...
        <td class="px-6 py-4 whitespace-nowrap text-sm">
          <a href="/payments/rent/${record.id}?month=${state.month}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-medium">
            View Details
          </a>
        </td>
      `;
  ...
  function updateCycleSummary() {
    if (!state.cycle) {
      ...
      if (elements.summary.propertyCount) {
        elements.summary.propertyCount.textContent = "-";
      }
      if (elements.summary.rentAmount) {
        elements.summary.rentAmount.textContent = "₹0.00";
      }
    ...
    if (elements.summary.propertyCount) {
      elements.summary.propertyCount.textContent = formatNumber(cycle.total_properties || state.records.length || 0);
    }

    if (elements.summary.rentAmount) {
      elements.summary.rentAmount.textContent = `₹${formatNumber(cycle.total_rent_amount || 0)}`;
    }
