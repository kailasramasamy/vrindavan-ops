...
export class RentPaymentCycleModel {
  static get db() {
    return db;
  }
  ...
  static async getCycleById(id, connection = null) {
    ...
    try {
      const [rows] = await db.query("SELECT * FROM rent_payment_cycles WHERE id = ?", [id]);
      if (!rows || rows.length === 0) {
        return { success: false, error: "Rent payment cycle not found" };
      }
      return { success: true, cycle: rows[0] };
    } catch (error) {
      console.error("RentPaymentCycleModel.getCycleById error:", error);
  ...
  static async getCycleByMonth(monthLike) {
    ...
    try {
      const [rows] = await db.query("SELECT * FROM rent_payment_cycles WHERE period_month = ?", [periodMonth]);
      if (!rows || rows.length === 0) {
        return { success: false, error: "Rent payment cycle not found" };
      }
      return { success: true, cycle: rows[0] };
    } catch (error) {
      console.error("RentPaymentCycleModel.getCycleByMonth error:", error);
      return { success: false, error: error.message };
    }
  }
  ...
  static async createCycle({ monthLike, startDate, endDate } = {}, connection = null) {
    ...
    try {
      const [result] = await db.query(
        `INSERT INTO rent_payment_cycles
      ...
      console.error("RentPaymentCycleModel.createCycle error:", error);
      return { success: false, error: error.message };
    }
  }
    ...
    try {
      const [rows] = await db.query(
        `SELECT 
          ...
          COALESCE(SUM(monthly_rent), 0) AS total_rent_amount,
      ...
      await db.query(
        `UPDATE rent_payment_cycles 
         ...
         WHERE id = ?`,
        [
          aggregates.total_properties || 0,
          aggregates.total_rent_amount || 0,
      ...
      console.error("RentPaymentCycleModel.updateCycleAggregates error:", error);
      return { success: false, error: error.message };
    }
  }
}

export default RentPaymentCycleModel;
