...
export class ElectricityPaymentCycleModel {
  static get db() {
    return db;
  }
  ...
  static async getCycleById(id, connection = null) {
    ...
    try {
      const [rows] = await db.query("SELECT * FROM electricity_payment_cycles WHERE id = ?", [id]);
      if (!rows || rows.length === 0) {
        return { success: false, error: "Electricity payment cycle not found" };
      }
      return { success: true, cycle: rows[0] };
    } catch (error) {
      console.error("ElectricityPaymentCycleModel.getCycleById error:", error);
  ...
  static async getCycleByMonth(monthLike) {
    ...
    try {
      const [rows] = await db.query("SELECT * FROM electricity_payment_cycles WHERE period_month = ?", [periodMonth]);
      if (!rows || rows.length === 0) {
        return { success: false, error: "Electricity payment cycle not found" };
      }
      return { success: true, cycle: rows[0] };
    } catch (error) {
      console.error("ElectricityPaymentCycleModel.getCycleByMonth error:", error);
      return { success: false, error: error.message };
    }
  }
  ...
  static async createCycle({ monthLike, startDate, endDate } = {}, connection = null) {
    ...
    try {
      const [result] = await db.query(
        `INSERT INTO electricity_payment_cycles
      ...
      console.error("ElectricityPaymentCycleModel.createCycle error:", error);
      return { success: false, error: error.message };
    }
  }
    ...
    try {
      const [rows] = await db.query(
        `SELECT 
         ...
         FROM electricity_payment_records
         WHERE cycle_id = ?`,
        [cycleId],
      );
      const aggregates = rows[0] || {};
      await db.query(
        `UPDATE electricity_payment_cycles 
         SET total_invoices = ?,
         ...
         WHERE id = ?`,
        [
          aggregates.total_invoices || 0,
          aggregates.total_invoice_amount || 0,
      ...
      console.error("ElectricityPaymentCycleModel.updateCycleAggregates error:", error);
      return { success: false, error: error.message };
    }
  }
}

export default ElectricityPaymentCycleModel;
