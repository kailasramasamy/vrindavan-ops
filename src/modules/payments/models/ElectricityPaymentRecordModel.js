...
export class ElectricityPaymentRecordModel {
  static get db() {
    return opsDb;
  }
  ...
  static async getRecordById(recordId, connection = null) {
    ...
    try {
      const [rows] = await db.query("SELECT * FROM electricity_payment_records WHERE id = ?", [recordId]);
      if (!rows || rows.length === 0) {
        return { success: false, error: "Electricity payment record not found" };
      }
      return { success: true, record: rows[0] };
    } catch (error) {
      console.error("ElectricityPaymentRecordModel.getRecordById error:", error);
  ...
  static async listRecords({ cycleId, limit = 200, offset = 0, status = null } = {}) {
    ...
    try {
      const [rows] = await db.query(
        `SELECT r.*,
         ...
         LEFT JOIN (
           ...
           FROM electricity_payment_entries
           GROUP BY record_id
         ) entry_counts ON entry_counts.record_id = r.id
         ${whereClause}
         ORDER BY r.invoice_date DESC, r.meter_name ASC
         LIMIT ? OFFSET ?`,
        [...params, l, o],
      );
      ...
      console.error("ElectricityPaymentRecordModel.listRecords error:", error);
      return { success: false, error: error.message };
    }
  }
  ...
  static async createRecord(recordData, connection = null) {
    ...
    try {
      ...
      const [result] = await db.query(
        `INSERT INTO electricity_payment_records
         (cycle_id, meter_name, meter_type, invoice_number, invoice_date, invoice_amount, total_adjustments, net_pay, payment_status, created_at, updated_at)
      ...
      console.error("ElectricityPaymentRecordModel.createRecord error:", error);
      return { success: false, error: error.message };
    }
  }
  ...
  static async updateRecordStatus(recordId, status, paymentDate = null) {
    ...
    try {
      ...
      await db.query(
        `UPDATE electricity_payment_records SET ${updates.join(", ")} WHERE id = ?`,
        [...params, recordId],
      );
      return this.getRecordById(recordId);
    } catch (error) {
      console.error("ElectricityPaymentRecordModel.updateRecordStatus error:", error);
      return { success: false, error: error.message };
    }
  }
  ...
  static async updateRecordRemarks(recordId, remarks) {
    ...
    try {
      await db.query(
        "UPDATE electricity_payment_records SET remarks = ?, updated_at = NOW() WHERE id = ?",
        [remarks || null, recordId],
      );
      return this.getRecordById(recordId);
    } catch (error) {
      console.error("ElectricityPaymentRecordModel.updateRecordRemarks error:", error);
      return { success: false, error: error.message };
    }
  }
  ...
  static async updateInvoiceDetails(recordId, invoiceData) {
    ...
    try {
      ...
      console.error("ElectricityPaymentRecordModel.updateInvoiceDetails error:", error);
      return { success: false, error: error.message };
    }
  }
  ...
  static async refreshRecordAggregates(recordId, connection = null) {
    ...
    try {
      ...
      const [recordRows] = await db.query(
        "SELECT invoice_amount FROM electricity_payment_records WHERE id = ?",
        [recordId],
      );
      ...
      await db.query(
        `UPDATE electricity_payment_records 
      ...
      console.error("ElectricityPaymentRecordModel.refreshRecordAggregates error:", error);
      return { success: false, error: error.message };
    }
  }
}

export default ElectricityPaymentRecordModel;
