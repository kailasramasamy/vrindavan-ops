...
export class ItServicesPaymentCycleModel {
  static get db() {
    return db;
  }
  ...
  static async getCycleById(id, connection = null) {
    ...
    try {
      const [rows] = await db.query("SELECT * FROM it_services_payment_cycles WHERE id = ?", [id]);
      if (!rows || rows.length === 0) {
        return { success: false, error: "IT services payment cycle not found" };
      }
      return { success: true, cycle: rows[0] };
    } catch (error) {
      console.error("ItServicesPaymentCycleModel.getCycleById error:", error);
  ...
  static async getCycleByMonth(monthLike) {
    ...
    try {
      const [rows] = await db.query("SELECT * FROM it_services_payment_cycles WHERE period_month = ?", [periodMonth]);
      if (!rows || rows.length === 0) {
        return { success: false, error: "IT services payment cycle not found" };
      }
      return { success: true, cycle: rows[0] };
    } catch (error) {
      console.error("ItServicesPaymentCycleModel.getCycleByMonth error:", error);
      return { success: false, error: error.message };
    }
  }
  ...
  static async createCycle({ monthLike, startDate, endDate } = {}, connection = null) {
    ...
    try {
      const [result] = await db.query(
        `INSERT INTO it_services_payment_cycles
      ...
      console.error("ItServicesPaymentCycleModel.createCycle error:", error);
      return { success: false, error: error.message };
    }
  }
    ...
    try {
      const [rows] = await db.query(
        `SELECT 
         ...
         FROM it_services_payment_records
         WHERE cycle_id = ?`,
        [cycleId],
      );
      const aggregates = rows[0] || {};
      await db.query(
        `UPDATE it_services_payment_cycles 
         SET total_invoices = ?,
      ...
      console.error("ItServicesPaymentCycleModel.updateCycleAggregates error:", error);
      return { success: false, error: error.message };
    }
  }
}

export default ItServicesPaymentCycleModel;
